...
var dirLightBeta = -utils.degToRad(-180);//120 X-axis
...
var currentShader = 0;                //Defines the current shader in use. 0
...
function createGLObjectModel(material, mesh) {
    ...
    let nTexture = false;
    let diffuseTextureObj = null;

    //Creating the vertex data.
    var UVFileNamePropertyIndex = -1;
    var diffuseColorPropertyIndex = -1;
    var specularColorPropertyIndex = -1;
    for (n = 0; n < material.properties.length; n++) {
        if (material.properties[n].key == "$tex.file") UVFileNamePropertyIndex = n;
        if (material.properties[n].key == "$clr.diffuse") diffuseColorPropertyIndex = n;
        if (material.properties[n].key == "$clr.specular") specularColorPropertyIndex = n;
    }
    ...
    for (var n = 0; n < mesh.vertices.length / 3; n++) {
        ...
        if (UVFileNamePropertyIndex >= 0) {
            objVertex.push(mesh.texturecoords[0][n * 2],
                mesh.texturecoords[0][n * 2 + 1]);
    ...
    if (UVFileNamePropertyIndex >= 0) {
        ...
        var imageName = material.properties[UVFileNamePropertyIndex].value;
        diffuseTextureObj = getTexture(imageName);
    }

    //*** mesh color
    let diffuseColor = material.properties[diffuseColorPropertyIndex].value || []; // diffuse value
    diffuseColor.push(1.0);													// Alpha value added
    let specularColor = material.properties[specularColorPropertyIndex].value;
...
function loadModel(modelName) {
    ...
    utils.get_json(modelsDir + modelName, function (loadedModel) {
        ...
        for (var i = 0; i < sceneObjects; i++) {
            ...
            facesNumber[i] = GLObjectModel.facesNumber;
            diffuseTextureObj[i] = GLObjectModel.diffuseTextureObj;
            nTexture[i] = GLObjectModel.nTexture;
            diffuseColor[i] = GLObjectModel.diffuseColor;
            specularColor[i] = GLObjectModel.specularColor;
            vertexBufferObjectId[i] = GLObjectModel.vertexBufferObjectId;
            indexBufferObjectId[i] = GLObjectModel.indexBufferObjectId;
    ...
    let keyFunctionUp = function (e) {
        if (keys[e.keyCode]) {
            ...
            switch (e.keyCode) {
                ...
                case KEY_CODE.I:
                    console.log("Info up");
                    testPaint = check_painting([cx, cy, cz]);
                    flag ^= 1;
                    break;
            }
        }
