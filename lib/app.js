...
function main() {
    ...
    if (gl) {
        ...
        // Open the json file containing the 3D model to load, parse it to retrieve objects' data and creates the VBO
        // and IBO from them. The vertex format is (x,y,z,nx,ny,nz,u,v)
        loadModel("museumTri.json");

        // loading info objects
        loadModel("infoObjects.json");

        //Load shaders' code, compile them, retrieve the handles
        loadShaders();
...
function get_light(element) {
    ...
    if (element.name === "self") {
        ...
        light.type = LIGHT_TYPE.POINT_W_DECAY;
    } else if (element.name === "a_sunday") {
        light.color = [1, 0.98, 0.85, 1.0];
        light.position = [-1.1755, 1.88744, 2];
        light.type = LIGHT_TYPE.POINT_W_DECAY;
    } else if (element.name === "the_bathers") {
        light.color = [1, 0.98, 0.85, 1.0];
        light.position = [-2.17547, 1.88744, 2];
        light.type = LIGHT_TYPE.POINT_W_DECAY;
    } else if (element.name === "starring_night") {
        light.color = [1, 0.98, 0.85, 1.0];
        light.position = [-3.29439, 1.88744, 2];
        light.type = LIGHT_TYPE.POINT_W_DECAY;
    } else if (element.name === "fourth_estate") {
        light.color = [1, 0.98, 0.85, 1.0];
        ...
        light.type = LIGHT_TYPE.POINT_W_DECAY;
    } else if (element.name === "Picasso_Guernica" || element.name === "Seurat_a_sunday"
        || element.name === "theBathers_Cezanne" || element.name === "starringNight"
        || element.name === "Volpedo_FourthEstate") {
        light.color = [1, 1, 1, 1.0];
        light.position = [-2.83391, 10, 2.13812];
        light.type = LIGHT_TYPE.POINT;
    } else if (element.name === "VanGogh_self" || element.name === "pisarro_boulevard_monmarte"
        || element.name === "Munch_Scream" || element.name === "Matisse_theDance"
        || element.name === "Monet-Sunrise" || element.name === "Manet_Dejeuner") {
        light.color = [1, 1, 1, 1.0];
