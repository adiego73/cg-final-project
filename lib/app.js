...
function get_light(element){
    ...
    if(element.name === "self"){
        alpha = 90;
        beta = -90;
        light.color = new Float32Array([1, 0, 1, 1.0]);
        light.position = [0.215, 2.1, 3.08];
        light.direction = [Math.cos(utils.degToRad(alpha)) * Math.cos(utils.degToRad(beta)), Math.sin(utils.degToRad(alpha)), Math.cos(utils.degToRad(alpha)) * Math.sin(utils.degToRad(beta))];
        light.type = 3;
...
function computeMatrices() {
    ...
    _.forEach(sceneObjects, (elem, index) => {
        ...
        elem.setObserverPosition(eyeTemp);
    });
}
