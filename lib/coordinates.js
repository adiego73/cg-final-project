...
function check_door(userCoor) {
    /*
     ...
     return true if you are inside the doors area
     */
    //User Coordinates
    var uX = userCoo[0];
    var uZ = userCoor[1];
    ...
    //left side door
    if (uZ > lDoorFront["ld"]["X"]  && uZ < lDoorFront["rd"]["X"]) {
...
function check_move(userCoor, room, museumFlag) {
    ...
    Z->Width and X->Depth
    return True or False if a move is valid
     */
    var invalidMoveX, invalidMoveZ; //Flags
    //User Coordinates
    var uX = userCoo[0];
    var uZ = userCoor[1];
    //Room Coordinates
    ...
    var bX1 = chamber1["ld"]["X"];
    var bX2 = chamber4["lu"]["X"];
    var bZ1 = chamber1["ld"]["Z"];
    var bZ2 = chamber5["rd"]["Z"];
    ...
    if (museumFlag) { //inside museum
        ...
        //Check in Z
        if ((uZ < gZ1 && uZ > bZ1) || (uZ > gZ2 && uZ < bZ2)) {
            //Moving in the outside frame of the museum
            invalidMoveZ = 0;
        } else {
            invalidMoveZ = 1
        ...
        //Check in X
        if ((uX < gX1 && uX > bX1) || (uX > gX2 && uX < bX2)) {
            //Moving in the outside frame of the museum
            invalidMoveX = 0;
        } else {
            invalidMoveX = 1
    ...
    return !(invalidMoveX || invalidMoveZ);
}
